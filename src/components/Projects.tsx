...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    const updateProgress = () => {
      ...
      const lastDot = containerRef.current.querySelector(`[data-timeline-dot="${data.projects.length - 1}"]`);
  ...
  return (
    <section id="projects" className="bg-transparent py-24">
      <div className="mx-auto max-w-6xl px-6">
        <p className="mb-2 text-xs font-bold uppercase tracking-[0.25em] text-[var(--color-accent)]">Portfolio</p>
        ...
        <div ref={containerRef} className="relative space-y-0">
          ...
          {data.projects.map((proj, idx) => {
            ...
            return (
              <div key={proj.id} className="relative pl-12 pb-12 last:pb-0">
                {/* Timeline dot — dynamic */}
                <div 
                  data-timeline-dot={idx}
                  className={`absolute left-0 top-2 h-[30px] w-[30px] rounded-full border-2 transition-all duration-300 flex items-center justify-center ${
                    isActive 
                ...
                <div className={`rounded-xl border transition-all duration-300 p-6 sm:p-8 ${
                  ...
                  {/* Header */}
                  <div className="mb-4">
                    <h3 className="text-lg font-bold text-[var(--color-text-primary)]">{proj.title}</h3>
